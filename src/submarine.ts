class Submarine {
  ...
  public update() {
    ...
    this.pickUpHullFix();
    this.allObjectsArray = this.obstacle.obstacles.concat(
      this.obstacle.powerUps
    );
  ...
  public collisionHullDamage() {
    for (let i = 0; i < this.allObjectsArray.length; i++) {
      if (
        this.allObjectsArray[i].collision &&
        this.allObjectsArray[i].id === "iceberg" &&
        this.hullHealth > 0
      ) {
      ...
      if (
        this.allObjectsArray[i].collision &&
        this.allObjectsArray[i].id === "mine" &&
        this.hullHealth > 0
      ) {
        this.hullHealth = 0;
      }
  ...
  public pickUpHullFix() {
    for (let i = 0; i < this.allObjectsArray.length; i++) {
      if (
        this.allObjectsArray[i].collision &&
        this.allObjectsArray[i].id === "supplyBox"
      ) {
